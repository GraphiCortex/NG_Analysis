...
# ---- Inputs / outputs (MAP2 integration) ----
IN_MERGED = os.path.join("Project", "New_Data_4", "binding_vs_Oshikawa_MAP2_expression.xlsx")
IN_GOXLS  = os.path.join("Project", "New_Data", "GO_results.xlsx")  # promoter GO workbook (Sh_2)
OUT_DIR   = os.path.join("Project", "New_Data_4", "figures", "map2")
os.makedirs(OUT_DIR, exist_ok=True)

FIGSIZE = (5.5, 4.0)

# 1) Load merged binding+expression table (sheet "All_genes")
merged = pd.read_excel(IN_MERGED, sheet_name="All_genes")
merged["Gene"] = merged["Gene"].astype(str).str.upper().str.strip()

# 2) Build category gene sets from GO workbook (Sh_2)
...
cats = {
    "Cell cycle":  os.path.join(OUT_DIR, "Scatter_Cell_cycle_map2.png"),
    "DNA repair":  os.path.join(OUT_DIR, "Scatter_DNA_repair_map2.png"),
    "Apoptosis":   os.path.join(OUT_DIR, "Scatter_Apoptosis_map2.png"),
    "Autophagy":   os.path.join(OUT_DIR, "Scatter_Autophagy_map2.png"),
    "Necroptosis": os.path.join(OUT_DIR, "Scatter_Necroptosis_map2.png"),
}

# 3) Shared axis limits across all categories for comparability
...
for cat, out_png in cats.items():
    ...
    ax.set_title(f"Binding vs. Expression (MAP2): {cat}", pad=8, fontsize=11)
    ax.set_xlabel("Binding log$_2$FC", fontsize=10)
    ax.set_ylabel("Expression log$_2$FC (MAP2)", fontsize=10)
